...
class EcoResetSubCommand extends BaseSubCommand {
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        $defaultBalance = (int)$config->getNested("settings.economy.starting-balance");

        if(!isset($args["amount"])) {
            $session->setBalance($defaultBalance);
            $sender->sendMessage(C::colorize("&r&aYou set " . $player->getName() . "'s balance to the server's default balance."));
            $player->sendMessage(C::colorize("&r&aYour balance has been reset to {$currency}" . number_format($defaultBalance) . "&a."));
        } else {
            ...
            $player->sendMessage(C::colorize("&r&aYour balance has been reset to {$currency}{$formattedAmount}&a."));
        }
    }

    public function getPermission(): string {
        return "aethelis.eco.reset";
    }
}